...
if __name__ == '__main__':
    ...
    for src in src_list:
        ...
        stg_path = 's3a://' + app_conf['s3_conf']['staging_lcoation'] + src
         if src == 'SB':
    ...
    txnDF.write.partitionBy('ins-dt').mode('overwrite').parquet(stg_path)
      elif src == 'OL':
       #sftp source
...
ol_txn_df.write.partitionBy('ins-dt').mode('overwrite').parquet()
    elif src =='CP'
...
students.show()
students.write.partitionBy('ins-dt').mode('overwrite').parquet()
    elif src =='ADDR'
#campaigns source
campaigns = spark \
             .read \
             .csv("s3://vamsibucket94/KC_Extract_1_20171009.csv")
             .withColumn('inst_dt',current_date())

 campaigns.show(5,false)
 campaigns.write.partitionBy('ins-dt').mode('overwrite').parquet()
